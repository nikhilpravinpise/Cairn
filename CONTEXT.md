...
A schema-valid `GeoLocation` used when the volunteer explicitly skips GPS or no fix is available.

Key invariants:
- `lat`: `0.0`
- `lng`: `0.0`
- `accuracy_m`: `0.0`
- `address_text`: `[GPS unavailable - location not recorded]`

Rationale: `location.accuracy_m` is constrained by
`docs/schema/evidence_packet_v1.schema.json` to be non-negative. Cairn must not
use negative numeric sentinels inside schema-constrained packet fields. If
future packets need richer absence semantics, add an explicit schema field
instead of overloading accuracy.

## Session Lifecycle

### Start

User begins a session. Location capture is optional and may produce a Skipped Location. Locale selection.
...
## Sprint 3 Architecture (Inference Image Policy + Orchestrator Depth)

### ImagePreprocessor

Seam at `lib/core/images/image_preprocessor.dart` that separates capture quality from inference quality. Original bytes in `SessionDraft` are **never** modified.

- `BoundedImagePreprocessor(maxLongEdgePx)` — production path. Downscales to longest edge ≤ `maxLongEdgePx`, aspect-ratio-preserving, via `dart:ui`. Images already within bound returned unchanged (zero cost).
- `PassthroughImagePreprocessor` — no-op. Used for raw-bytes benchmarks and unit tests.

`ModelSpec.inferenceMaxLongEdgePx` (default **768** for both `e2b` and `e4b`) drives which `BoundedImagePreprocessor` is wired in `orchestratorProvider`.

### describeAll Stream

`GemmaOrchestrator.describeAll(List<DescribePhotoRequest>)` owns the full batch inference lifecycle. Emits a sealed `DescribePhotoEvent` per photo:

| Event | When | Key fields |
|---|---|---|
| `DescribePhotoStarted` | Before model call | `request`, `index`, `total` |
| `DescribePhotoSucceeded` | After valid response | `result` (`DescribePhotoResult`), `turn` (`TurnRecord`) |
| `DescribePhotoFailed` | On any error | `request`, `error` |

Errors are isolated per-photo — one failure never aborts the batch. The orchestrator builds `TurnRecord`; callers just pass it to `SessionController.recordTurn`.

### TurnRecord

Extracted to `lib/core/llm/turn_record.dart`. Re-exported from both `orchestrator.dart` and `session_controller.dart` to keep existing call-sites unchanged.

### DescribePhotoRequest

`lib/core/llm/orchestrator.dart`. Input type for `describeAll`; carries `observationId`, `promptId`, `askedIn`, `imageBytes`, `imageRef`, optional `userText`. Observation IDs are generated by the UI **before** building requests, ensuring deterministic allocation.
