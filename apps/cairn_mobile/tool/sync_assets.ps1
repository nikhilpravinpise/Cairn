...
if (-Not (Test-Path $src_prompt_vision)) { Write-Error "missing $src_prompt_vision"; exit 1 }
if (-Not (Test-Path $src_prompt_vision_agg)) { Write-Error "missing $src_prompt_vision_agg"; exit 1 }
...
Copy-Item $src_prompt_vision $dst_prompt_vision -Force
Copy-Item $src_prompt_vision_agg $dst_prompt_vision_agg -Force
Copy-Item $src_schema $dst_schema -Force

Write-Host "synced $dst_prompt"
Write-Host "synced $dst_prompt_vision"
Write-Host "synced $dst_prompt_vision_agg"
